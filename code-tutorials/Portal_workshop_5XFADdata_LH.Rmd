...
We will also use the BioconductoR package manager to install `biomaRt`.
...
```{r}
#create a dataframe of the specimenIDs in the study from the counts matrix column headers, removing the first column named "gene_id"
specimenID <- data.frame(specimenID = colnames(counts)[-1])

# join the specimenIDs from the counts matrix to the biospecimen metadata, keeping only rows in the biospecimen metadata that have a match in the RNAseq assay metadatda and filtering out specimens used in other assays
IDkey <- dplyr::left_join(specimenID, bio_meta, by="specimenID")

# all these specimens are from the same organ and tissue, so we will remove the biosepcimen metadata variables except for individualID
IDkey <- subset(IDkey, select=c(individualID, specimenID))

# we can join this IDkey dataframe to the individual metadata by individualID, and later to the counts data by specimenID
ind_meta2 <- dplyr::left_join(IDkey, ind_meta, by="individualID")
```

The MODEL-AD individual mouse metadata contains columns with birth date and death date for each mouse. Rather than deal with converting the strings in these columns to dttm values and subtracting birth from death to get the individual's age at death, we will use information in the "jobNumber" column to pull out simplified ages.
...
Based on this table, we have "jobNumber" categories for 4, 6, and 12 timepoints, plus a category labelled "4/6 month backups". The **RNASeq methods description** from the [Jax 5XFAD study page in the AD Portal](https://adknowledgeportal.synapse.org/Explore/Studies/DetailsPage?Study=syn21983020) indicates that these individuals belong to the 4 month timepoint. We will add a new column to the dataframe with simplified time points based on the values in the jobNumber column -- e.g., `jobNumber = 'JAX 5xFAD 12mo'` will be `timepoint = '12mo'`.
...
```{r}
table(ind_meta2$sex, ind_meta2$timepoint, ind_meta2$genotype)
```
...
We will use the BioconductoR package manager to install `biomaRt`.
...
We will use two custom functions to convert ensemblIDs to gene names.
...
```{r}
#save the original counts matrix for later & make a copy for more manipulating:
counts2 <- counts
counts2$gene_short_name <- Make.Gene.Symb(counts2$gene_id)
```

We need to clean up the gene names a little, and assign them as row names of the data frame.
...
counts2[1, "gene_short_name"] <- "PSEN1"
counts2[2, "gene_short_name"] <- "APP"

#make all gene names unique and assign them as row names:
counts2$gene_short_name <- make.unique(counts2$gene_short_name)
rownames(counts2) <- counts2$gene_short_name

#delete unnecessary columns
counts2$gene_id<-NULL
counts2$gene_short_name<-NULL
```

Now we can transpose the dataframe so that each row contains count data cross all genes for an individual, and join our individual metadata by specimenID.

```{r}
#transpose the matrix, so genes are columns and individuals are rows
counts2 <- as.data.frame(t(counts2))

#add back a specimenID column to the dataframe
counts2$specimenID <- row.names(counts2)

# join the transposed data frame to our filtered individual metadata
...
Examine any gene of interest by setting the y argument in the `ggplot(aes()` mapping equal to the gene name. Ex: `y = CstJ`
...
Plot Apoe expression by animal weight

```{r}

ggplot(counts2, aes(x = log10(brainWeight), y = log10(Apoe), color = sex)) +
  geom_point()
```
