...
You can use the original counts matrix and joined metadata you uploaded in the Portal_workshop_5XFADdata.Rmd
```{r}
...
head(counts)
rownames(ind_meta2) <- ind_meta2$specimenID  #use metadata file you created that contains specimenID
head(ind_meta2)
```
...
metadata <- ind_meta2[sort(rownames(ind_meta2)),]

meta.12M.Male <- metadata[(metadata$sex=="male" & metadata$timepoint=='12mo'),]
